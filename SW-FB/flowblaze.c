...
#define RTE_LOGTYPE_FLOWBLAZE RTE_LOGTYPE_USER1
#define PROGNAME "flowblaze"
...
fb_worker_main_loop(void)
{
    ...
    FILE *file;
    cJSON *json = NULL, *stages = NULL, *s;
    char *string = NULL;
    long file_size = 0;
    xfsm_pipeline_t *pipeline;
    int curr_stage = 0;
#ifdef CACHE_ENABLE
    cache_key_t *ck;
    ...
    RTE_LOG(INFO, FLOWBLAZE, "entering main loop on lcore %u\n", lcore_id);
    ...
    string = malloc((size_t ) (file_size + 1));
    fread(string, file_size, 1, file);
    fclose(file);

    pipeline = rte_zmalloc("pipeline", sizeof(xfsm_pipeline_t), CACHE_LINE_SIZE);
    pipeline->tables = rte_zmalloc("stages", sizeof(xfsm_table_t *) * MAX_STAGES, CACHE_LINE_SIZE);
    ...
    cJSON_ArrayForEach(s, stages) {
        cJSON *table = cJSON_GetObjectItemCaseSensitive(s, "table");
        pipeline->tables[pipeline->num_stages] = json_to_table(table);
        pipeline->num_stages++;
    }
    ...
    while (!force_quit) {
        ...
        if (unlikely(diff_tsc > drain_tsc)) {
            ...
            printf("nb_instances: %"PRIu32"\tcache_matches: %lu\n", pipeline->tables[0]->nb_instances, cache_matches);
        ...
        for (i = 0; i < nb_rx_ports; i++) {
            ...
            for (j = 0; j < nb_rx; j++) {
                ...
                while (curr_stage != NULL_STAGE) {
                    xfsm = pipeline->tables[curr_stage];

                    xfsm->curr_pkt = m;
                    xfsm->in_port = (uint8_t) (i + 1);
                    xfsm->curr_pkt->timestamp = (cur_tsc * US_PER_S) / rte_get_tsc_hz();
                    parse_pkt_header(xfsm, m);

                    if (key_extract(xfsm, fk)) {
                        RTE_LOG(INFO, FLOWBLAZE, "FK: %"PRIX32",%"PRIX32"\n", fk->flds[0], fk->flds[1]);

                        match_inst = lookup_instance(xfsm, fk);

                        RTE_LOG(INFO, FLOWBLAZE, "\nGlobal registers [%"PRIi64", %"PRIi64", %"PRIi64", %"PRIi64"]\n"
                                                 "Local registers [%"PRIi64", %"PRIi64", %"PRIi64", %"PRIi64"]\n",
                                                 xfsm->global_registers[0], xfsm->global_registers[1],
                                                 xfsm->global_registers[2], xfsm->global_registers[3],
                                                 match_inst->registers[0], match_inst->registers[1],
                                                 match_inst->registers[2], match_inst->registers[3]);
                        ...
                        curr_stage = match->next_stage;
                    }
                }
                free:
                rte_pktmbuf_free(m);
                curr_stage = 0;     // reset current stage to 0
            }
        }
    }
}
...
static void
usage (char *name) {
    printf("Usage: ./%s [DPDK EAL arguments] -- [FlowBlaze arguments]\n"
           "FlowBlaze arguments:\n"
           "\t-f file_path.json\n",
           name);
}
...
static int
flowblaze_parse_args(int argc, char **argv)
{
    int opt, ret;
    char **argvopt;
    char *prgname = argv[0];
    ...
    while ((opt = getopt_long(argc, argvopt, short_options, longopts, &longindex)) != EOF) {

        switch (opt) {
            case 'f':
                strncpy(file_path, optarg, strnlen(optarg, 63));
                break;
            case ':':
...
main(int argc, char **argv)
{
    ...
    if (argc == 1)
        rte_exit(EXIT_FAILURE, "Please, provide FlowBlaze specific arguments\n");

    // parse arguments
    flowblaze_parse_args(argc, argv);
    ...
    RTE_ETH_FOREACH_DEV(portid) {
        ...
        /* init port */
        RTE_LOG(INFO, FLOWBLAZE, "Initializing port %u... ", portid);
        //fflush(stdout);
    ...
    RTE_ETH_FOREACH_DEV(portid) {
        RTE_LOG(INFO, FLOWBLAZE, "Closing port %d...", portid);
        rte_eth_dev_stop(portid);
        rte_eth_dev_close(portid);
        RTE_LOG(INFO, FLOWBLAZE, " Done\n");
    }
    RTE_LOG(INFO, FLOWBLAZE, "Bye...\n");
