...
#define RTE_LOGTYPE_RSS_TEST RTE_LOGTYPE_USER1
...
// SYMMETRIC HASH KEY
static uint8_t hash_key[RSS_HASH_KEY_LENGTH] = {0x6D, 0x5A, 0x6D, 0x5A, 0x6D, 0x5A, 0x6D,
                                                0x5A, 0x6D, 0x5A, 0x6D, 0x5A, 0x6D, 0x5A,
...
fb_worker_main_loop(void)
{
    ...
    unsigned i, j, nb_rx, nb_rx_ports, queue_id;
    ...
    printf("queue id of lcore %u is %u\n", lcore_id, queue_id);

    RTE_LOG(INFO, RSS_TEST, "entering main loop on lcore %u\n", lcore_id);
    ...
    while (!force_quit) {
        ...
        if (unlikely(diff_tsc > drain_tsc)) {
            ...
            for (i = 0; i < qconf->n_rx_port; i++) {
                ...
                rte_eth_tx_buffer_flush((uint16_t) i, (uint16_t) queue_id, buffer);
            }
        ...
        for (i = 0; i < nb_rx_ports; i++) {
            nb_rx = rte_eth_rx_burst((uint16_t ) i, (uint16_t ) queue_id, pkts_burst, MAX_PKT_BURST);
            if (nb_rx > 0) {
                RTE_LOG(INFO, RSS_TEST, "Received packet on lcore %u\n", lcore_id);
            }
            ...
            for (j = 0; j < nb_rx; j++) {
                ...
                if (key_extract(xfsm, fk)) {
                    RTE_LOG(DEBUG, RSS_TEST, "FK: %"PRIX32",%"PRIX32"\n", fk->flds[0], fk->flds[1]);
...
main(int argc, char **argv)
{
    ...
    RTE_ETH_FOREACH_DEV(portid) {
        ...
        ret = rte_eth_dev_configure(portid, nb_lcores, nb_lcores, &local_port_conf);
        if (ret < 0)
        ...
        tx_buffer[portid] = rte_zmalloc_socket("tx_buffer",
                                               RTE_ETH_TX_BUFFER_SIZE(NB_TX_PKTS), 0,
                                               rte_eth_dev_socket_id(portid));
        if (tx_buffer[portid] == NULL)
            rte_exit(EXIT_FAILURE, "Cannot allocate buffer for tx on port %u\n",
                     portid);

        rte_eth_tx_buffer_init(tx_buffer[portid], NB_TX_PKTS);
